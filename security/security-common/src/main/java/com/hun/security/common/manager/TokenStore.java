...
import com.hun.common.constants.OauthCacheNames;
import com.hun.common.exception.BusinessException;
import com.hun.common.util.PrincipalUtil;
...
public class TokenStore {
    ...
    private int getExpiresIn(int sysType) {
        ...
        // 系统管理员的token过期时间 2小时
        if (Objects.equals(sysType, SysTypeEnum.Employee.value())) {
            expiresIn = expiresIn * 24 * 30;
        }
        //过期时间一个月.
        return expiresIn;
    ...
    public UserInfoInTokenBO getUserInfoByAccessToken(String accessToken, boolean needDecrypt) {
        if (StrUtil.isBlank(accessToken)) {
            throw new BusinessException("accessToken is blank");
        }
        ...
        if (userInfoInTokenBO == null) {
            throw new BusinessException("accessToken 已过期");
        }
